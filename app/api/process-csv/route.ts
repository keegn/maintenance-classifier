...
async function isEmergency(text: string, model: string): Promise<boolean> {
  try {
    const prompt = text
      ...
      : `You are Jerry from Stonehaven Property Management after hours phone line. You are an expert customer service rep in the field of multifamily apartment maintenance. You are pleasant and friendly, understanding that the resident calling might be in distress due to a maintenance emergency related to their rented apartment. Begin the call with "Hello, this is Jerry from Stonehaven Property Management Maintenance Division. How can I assist you?"
...
- Very fast running water leak, flood, or HVAC failure when outside temperature is below 40°F or above 90°F.
- Remember that emergencies mean that tenants are not charged and do not mention a charge or cost. 
...
- No hot water (e.g. hot water isn't working but cold water is)
...
async function processCsv(model: string) {
  const rows: any[] = []

  return new Promise<void>((resolve, reject) => {
    const filePath = 'app/api/process-csv/maintenance_requests.csv' // Relative path to the CSV file
        ...
        try {
          for (const row of rows) {
            ...
            if (emergencyRequest) {
              ...
              apiResult ? truePositive++ : falseNegative++
              if (!apiResult) falseNegativeText.push(emergencyRequest)
            }

            if (nonEmergencyRequest) {
              nonEmergencyCount++
              const apiResult = await isEmergency(nonEmergencyRequest, model)
              apiResult ? falsePositive++ : trueNegative++
              if (apiResult) falsePositiveText.push(nonEmergencyRequest)
            }
          }
